...
from solutions.filtering_np_gameplan import ExplorationGameplanNP

# ROTATION VELOCITY = 1.0 => ANGLE STEP = 0.2862392814389153


class FilteredDrone(DroneAbstract):

    # DEFINITIONS OF ATTRIBUTES (including the np map)
    ...
    def __init__(self, identifier: Optional[int] = None, **kwargs):
        ...
        self._exploration_gameplan = ExplorationGameplanNP()
        self.map_explored_drone= np.zeros((1500, 1500))


    def display_drone_map(self):
        
        map_rgb = np.zeros((1500, 1500, 3))
        for i in range(self.map_explored_drone.shape[0]):
            for j in range(self.map_explored_drone.shape[1]):
                if self.map_explored_drone[i, j] == 0:  # Unexplored
                    map_rgb[i, j] = [0, 0, 0]
                elif 1 <= self.map_explored_drone[i, j] <= 2:  # Unknown obstacle
                    map_rgb[i, j] = [255, 255, 255]
                elif self.map_explored_drone[i, j] == 3:  # Wounded
                    map_rgb[i, j] = [255, 0, 0]
                elif self.map_explored_drone[i, j] == 4:  # Void explored
                    map_rgb[i, j] = [80, 80, 80]
        plt.imshow(map_rgb.astype('uint8'), origin='lower')
        plt.show()
    ...
    def process_communication_sensor(self):
        ...
        def process_gameplan(self):
            ...
            self._exploration_gameplan.update_map(self.measured_position(),
                                       self.measured_angle(),
                                       self.lidar().get_sensor_values(),
                                       self.semantic_cones().sensor_values)

    def process_gameplan(self):
        #measured_positions() measured_angled vs true_position() true_angle()
        self.map_explored_drone = self._exploration_gameplan.update_map(self.true_position(), 
                                                                        self.true_angle(), 
                                                                        self.lidar().get_sensor_values(),
                                                                        self.semantic_cones().sensor_values)
